...
SUPPORTED_SUBTITLE_EXTENSIONS=('srt' 'ssa')
...
process_subtitle_file() {
	local FILENAME="$1"
	DESTINATION_SUBTITLE_FILENAME="$FILENAME".vtt
	GENERATED=false
	for SUB_EXTENSION in "${SUPPORTED_SUBTITLE_EXTENSIONS[@]}"
	do
		SUBTITLE_FILENAME="$FILENAME".$SUB_EXTENSION
	        if [ -f "$SUBTITLE_FILENAME" ]; then
			SUB_ENCODING=$(file -b --mime-encoding "$SUBTITLE_FILENAME")
			$FFMPEG -loglevel error -y -sub_charenc $SUB_ENCODING -i "$SUBTITLE_FILENAME" "$DESTINATION_SUBTITLE_FILENAME"
			GENERATED=true
		fi
	done
        if [ $GENERATED == true ]; then
		echo "- generated subtitle $DESTINATION_SUBTITLE_FILENAME"
	fi
}
...
process_file() {
	...
	process_subtitle_file "${FILENAME%.*}"
